...
for map in maps
    ...
    selected = rand(1:map[1]*map[2], rock_nums)
    for pos in selected
...
for pomdp in pomdps
    # QMDP upper bound
    @everywhere qmdp_policy = solve(QMDPSolver(), $pomdp)
    ...
    # For LB-DESPOT
        ...
        bounds = IndependentBounds(DefaultPolicyLB(to_best), 100.0, check_terminal=true)
    ...
    CSV.write("RockSample_DESPOT.csv", dfs[1])
    CSV.write("RockSample_UCT_DESPOT.csv", dfs[1])
    CSV.write("RockSample_POMCP.csv", dfs[3])
end